...
lemma isin_node [simp]: "isin_trie (node b lr) ks = isin_trie(Nd b lr) ks"
  sorry
...
lemma isin_insert: "isin_trie (insert_trie xs t) ys = (xs = ys \<or> isin_trie t ys)" 
  ...
  subgoal for b t1 t2 ys
    apply(cases ys)
    using set_trie_def stit_32 apply force
  ...
  proof -
    ...
    (* by (smt (z3) Defs.trie.distinct(5) Defs.trie.inject Submission.insert_trie.simps(5) Submission.isin_trie.elims(1) Submission.isin_trie.simps(1) Submission.isin_trie.simps(4) a_1 a_2 mem_Collect_eq node_def set_trie_def stit_3) *)
    proof(cases x)
      case True
      then show ?thesis 
      proof(cases ys)
      ...
      next
        case (Cons a list)
        then show ?thesis
          using True a_1 node_def by force
      qed 
    next
      case False
      then show ?thesis 
      ...
      next
        case (Cons a list)
        then show ?thesis
          using False a_2 node_def by force
      qed 
    qed
  qed
