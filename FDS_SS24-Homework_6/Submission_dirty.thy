...
lemma exists_simple_path:
  ...
  from 1 show "\<exists>p'. path G u p' v \<and> distinct p'" 
  proof(cases "distinct xs")
    case True
    then show ?thesis using "1.prems" by blast
  next
    case False
    obtain ys zs as y where xs_split:  "xs = ys @ y # zs @ y # as" using False not_distinct_decomp[of "xs"] by auto
...
declare [[names_short]]
theorem compnet_msett: "mset (run_compnet comps xs) = mset xs" unfolding run_compnet_def compnet_step_def
  apply(induction comps arbitrary: xs) 
  apply(simp)
...
lemma compnet_step_mono: 
  ...
  apply(induction xs) 
  apply(simp)
  by (smt (verit, del_insts) assms linorder_le_less_linear list_update_same_conv map_update monoD mono_invE order_less_le)

lemma compnet_step_monoo: 
  assumes "mono f" 
  shows "compnet_step c (map f xs)  = map f (compnet_step c  xs)"
  apply(simp add: compnet_step_def) 
  apply(auto)
...
next
  case (Cons a cs)
  (*have "fold compnet_step (a#cs) (map f xs) = map f (fold compnet_step (a#cs) xs)" using compnet_step_mono fold_simps(2)
    by (metis Cons assms run_compnet_def)*)
  then show ?case unfolding run_compnet_def by (simp add: assms compnet_step_monoo) 
qed

find_consts "'a list \<Rightarrow> 'a \<Rightarrow> nat"

find_theorems "List.nth"
...
proof (rule ccontr)
  ...
  from this have "\<exists> k. (k < (length (run_compnet cs xs)) - 1) \<and> (nth (run_compnet cs xs) k) >  (nth (run_compnet cs xs) (k + 1))"
  ...
  let ?f = "\<lambda>x. (if x > ?a_k1 then True else False)"
  ...
  have "?a_k' = ?f (nth (run_compnet cs xs) k)" using k_len
    by simp
  from this have a_k_t:  "?a_k' = True" using g 
    by simp
  ...
  from a_k_t a_k1_f have "\<not> sorted (map ?f (run_compnet cs xs))"
    using k_len sorted_wrt_nth_less by fastforce
  from this have  "\<not> sorted (run_compnet cs (map ?f xs))" using f_mon
    by simp

  from this show False using assms 
    by simp
qed 
