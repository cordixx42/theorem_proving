...
lemma rev_pre_butlast: "x \<le> (length (butlast xs)) \<Longrightarrow> rev_pre x (butlast xs) = butlast (rev_pre x xs)" 
  apply(induction xs  rule: rev_pre.induct)
  apply(auto)
  apply (simp add: butlast_conv_take)
  by (simp add: butlast_append drop_butlast take_butlast)


lemma rev_pre_chain_butlast: "(\<forall> x \<in> (set xs). x \<le> (length (butlast ys))) \<Longrightarrow> (rev_pre_chain xs (butlast ys)) = butlast (rev_pre_chain xs ys)"
...
next
  ...
  also have "... =  rev_pre_chain xs  (butlast (rev_pre a ys))" using rev_pre_butlast Cons
...
lemma index_max_upper_bound: "xs \<noteq> [] \<Longrightarrow> (Suc (index (xs) (Max (set (xs))))) \<le> length (xs)"
  apply(induction xs)
  apply(simp)
  apply(simp)
...
lemma psort_revs_upper_bound: "\<forall> x \<in> set (psort_revs xs). x \<le> length xs"
...
next
  ...
  have c: "(Suc (index (v#va) (Max (set (v#va))))) \<le> length (v#va)" using index_max_upper_bound
    by blast 
...
lemma last_same:  "(\<forall> x \<in> (set xs). x \<le> (length (butlast ys))) \<Longrightarrow> last (rev_pre_chain xs ys) = last ys"  
...
next
  ...
  have len: "\<forall> x \<in> set ((psort_revs (butlast yss))). x \<le> length (butlast yss)" using psort_revs_upper_bound 
    by blast  

  from this  have "butlast (rev_pre_chain 
                  (psort_revs (butlast yss))
                  (yss)) = rev_pre_chain  (psort_revs (butlast yss)) (butlast yss)" using rev_pre_chain_butlast 
    by force 
  ...
  have k:  "last (rev_pre_chain 
                  (psort_revs (butlast yss))
                  (yss)) = last yss" using len last_same
    by blast
