...
fun bv_invar :: "bool list \<Rightarrow> bool " where 
 "bv_invar xs =  (xs = [] \<or> last xs = True)" \<comment> \<open>@{lemma "\<And>A . A=True \<longleftrightarrow> A" by simp}\<close>
...
"bv_delete_n x xs = rev (dropWhile (\<lambda>k. k = False) (rev  (xs[x := False])))" \<comment> \<open>@{lemma "(\<lambda>k. k = False) = Not" by simp}\<close>
...
\<comment> \<open>Somehow this feels like it could have been an auto call to me, but I do not have the time to compress it :(\<close>
...
lemma set_bv_all_false: 
  assumes  "zs = [] \<or> (\<forall> x \<in> set zs. x = False)" \<comment> \<open>@{lemma "zs = [] \<or> (\<forall> x \<in> set zs. x = False) \<longleftrightarrow> True \<notin> set zs" by auto}\<close>
...
\<comment> \<open>What do you need to know? Basically you can write \<^term>\<open>\<And>x . P x \<Longrightarrow> Q x\<close> in an Isar proof as
  \<open>"Q x" if "P x" for x\<close>, \<open>P x\<close> available locally under the name \<open>that\<close>.\<close>
...
\<comment> \<open>Seems ok, unused\<close>
...
\<comment> \<open>Also holds without \<open>+1\<close>, seems useful\<close>
...
lemma "set_itree2 (joinn l r) = set_itree2 l \<union> set_itree2 r"
  apply (induction l r rule: joinn.induct)
    apply (fastforce split: itree.splits)+ done \<comment> \<open>Sanity 1, problem: It gets lost in tuples easily, as you never look at them it might be worth not 
    showing the members in the definition\<close>

lemma "set_itree3 (joinn l r) = set_itree3 l \<union> set_itree3 r"
  apply (induction l r rule: joinn.induct)
    apply (fastforce split: itree.splits)+
  done \<comment> \<open>Sanity 2, see bove\<close>
...
\<comment> \<open>Oops, should have looked down\<close>
...
\<comment> \<open>Probably true, easier to state without the \<^const>\<open>iNode\<close> constructor, instead write what exactly needs to hold about x,y\<close>
...
(* helper lemmas *) 
\<comment> \<open>It might be easier to talk about the actual sets produced\<close>
...
\<comment> \<open>Somewhat overly detailed, but nice job!\<close>
...
\<comment> \<open>Not following hint, but that is not required\<close>

lemma itree2_del_2: "ibst (iNode l (a,b) r) \<Longrightarrow>  (\<forall> y \<in> set_itree2 (delete x r). b < y)"
proof - 
  assume ass:  "ibst (iNode l (a,b) r)" \<comment> \<open>Long goal format\<close>
...
\<comment> \<open>Still around +4\<close>
           ...
           next 
             ...
             next 
               ...
               then have  "(delete x (iNode l (a, b) r)) = (iNode l (a,x-1) (insert_min r (x+1,b))) " using 2 b d f h
                 by simp
               show  "ibst (delete x (iNode l (a, b) r))" using 2 b d f h apply (auto simp add: if_splits)
                  \<comment> \<open>Insert min case\<close>
                 sorry
             qed  
    next 
    qed
